# 通常kill処理
execute as @a[team=gold] at @s run playsound minecraft:entity.experience_orb.pickup master @s ~ ~ ~ 1 1

execute if score @s pvp_advance matches 1 run tellraw @a[team=gold] [{"bold":false,"color":"gray","text":"["},{"bold":false,"color":"aqua","text":"!"},{"bold":false,"color":"gray","text":"] "},{"bold":false,"color":"gold","selector":"@s"},{"bold":true,"color":"green","text":"が敵チームのメンバーをKILLした！"},{"bold":false,"color":"aqua","text":"(+1)"}]
execute if score @s pvp_advance matches 1 run scoreboard players add 金鉱石 PvP_points 1
...
advancement revoke @s only pvp:kill/gold
