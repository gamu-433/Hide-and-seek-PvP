# 通常kill処理
execute as @a[team=lapis_lazuli] at @s run playsound minecraft:entity.experience_orb.pickup master @s ~ ~ ~ 1 1

execute if score @s pvp_advance matches 1 run tellraw @a[team=lapis_lazuli] [{"bold":false,"color":"gray","text":"["},{"bold":false,"color":"aqua","text":"!"},{"bold":false,"color":"gray","text":"] "},{"bold":false,"color":"gold","selector":"@s"},{"bold":true,"color":"green","text":"が敵チームのメンバーをKILLした！"},{"bold":false,"color":"aqua","text":"(+1)"}]
execute if score @s pvp_advance matches 1 run scoreboard players add ラピスラズリ PvP_points 1
...
advancement revoke @s only pvp:kill/lapis_lazuli
