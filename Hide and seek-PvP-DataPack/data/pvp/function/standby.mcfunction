...
scoreboard players set @a[scores={pvp_advance=1}] PvP_Item_ore 100
execute as @a at @s run playsound minecraft:custom.levelup master @s ~ ~ ~ 1 1
execute store result score 人数 pvp_advance run tellraw @a[scores={pvp_advance=1}] {"text":"1分後にスタートします！","color":"green","bold":true}
...
effect give @a[scores={pvp_advance=1}] minecraft:saturation 60 1 true
effect give @a[scores={pvp_advance=1}] minecraft:instant_health 60 1 true
execute as @a[scores={pvp_advance=1}] at @s unless entity @s[tag=no_clear_item] run clear @s